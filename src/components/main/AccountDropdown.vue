...
async function getProfile() {
  axios
    ...
    .then((response) => {
      ...
      profile.value.birth_date = response.data["birth_date"]
        .split("-")
        .reverse()
        .join(".");
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <label> <b>Имя: </b>{{ profile.first_name }} </label>
          <label> <b>Фамилия: </b>{{ profile.last_name }} </label>
          <label> <b>Дата рождения: </b>{{ profile.birth_date }} </label>
        </div>
      </div>
    </div>
  </div>
